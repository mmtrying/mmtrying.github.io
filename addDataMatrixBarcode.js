...
function addDataMatrixBarcode( Q ) {
  ...
  toEdifact = function( t ){
    var n = t.length;
    var l = ( n + 1 ) & -4, cw = 0, ch;
    var r = ( l > 0 ) ? [ 240 ] : [];   /* switch to Edifact */
  ...
  toText = function( t, s ){
    var i, j, cc = 0, cw = 0, l = t.length, r = [ s[ 0 ] ];   /* start switch */
    var push = function( v ){ 
 ...
 return ( function(){
  ...
  var abs = Math.abs, r, x, y, d, sx, sy;
  var ns = 'http://www.w3.org/2000/svg', path = '';
  var q = ('string' == typeof Q ) ? { msg: Q } : Q || {};
  var p = q.pal || ['#000'];
  var dm = abs( q.dim ) || 256;
  var pd = abs( q.pad ), pd = ( pd > -1 ) ? pd : 2;
  var mx = [ 1, 0, 0, 1, pd, pd ];

  var fg = p[ 0 ], fg = ishex( fg ) ? fg : '#000';
  var bg = p[ 1 ], bg = ishex( bg ) ? bg :  0;
