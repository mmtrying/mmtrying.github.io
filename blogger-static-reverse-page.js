<div id="staticpostwrapper"><div class="staticpostwrapper"><ul id="staticpost"></ul></div></div>
...
//Before this scripting, please be remined to add "<div id="staticpostwrapper"><div class="staticpostwrapper"><ul id="staticpost"></ul></div><div id="staticfooter"></div></div>" to your html content.
...
function rssFeedCallBack(i){
    ...
    if(ThisIdx != NextIdx){
        ...
        s.src=staticPageSetting.blog_domain_url+'/feeds/posts/default?tt='+$thisTime+'&orderby='+staticPageSetting.post_order+'&alt=json-in-script&max-results=0&start-index=1&callback=addingstaticpagepost';
    }
}
...
function addingstaticpagepost($fakeJson){
   var $jL = parseInt($fakeJson.feed.openSearch$totalResults.$t)||0;
   var $max = staticPageSetting.post_limit;
   var $start = $jL-($max*(parseInt($staticPageIndexRecord[1])||1))-staticPageSetting.start_index+2;
   for(var i=$start;i<1;i++){
    ...
    $s.type='text/javascript';
    $s.src=staticPageSetting.blog_domain_url+'/feeds/posts/default?tt='+$thisTime+'&orderby='+staticPageSetting.post_order+'&alt=json-in-script&max-results='+$max+'&start-index='+$start+'&callback=staticpagepostwidget';
}

(function(){
document.write('<script type=\"text/javascript\" src=\"'+staticPageSetting.blog_domain_url+'/feeds/posts/default?tt='+$thisTime+'&orderby='+staticPageSetting.post_order+'&alt=json-in-script&max-results=0&start-index=1&callback=addingstaticpagepost"><\/script>');
})();
</script>
